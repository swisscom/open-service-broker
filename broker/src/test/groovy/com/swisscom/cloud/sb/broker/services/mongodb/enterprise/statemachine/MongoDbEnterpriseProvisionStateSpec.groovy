...
class MongoDbEnterpriseProvisionStateSpec extends Specification {
    ...
    def "CREATE_OPS_MANAGER_GROUP"() {
        ...
        helper.getValue(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_GROUP_ID) == opsManagerGroup.groupId
        helper.getValue(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_GROUP_NAME) == opsManagerGroup.groupName
        helper.getValue(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_AGENT_API_KEY) == opsManagerGroup.agentApiKey
        helper.getValue(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_HEALTH_CHECK_USER)
        helper.getValue(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_HEALTH_CHECK_PASSWORD)
    }
    ...
    def "CHECK_AGENTS"() {
        ...
        def groupId = 'GroupId'
        def agentCount = 3
        context.lastOperationJobContext = new LastOperationJobContext(serviceInstance: new ServiceInstance(details: [ServiceDetail.from(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_TARGET_AGENT_COUNT, agentCount.toString()),
                                                                                                                     ServiceDetail.from(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_GROUP_ID, groupId
                                                                                                                     )]))
        and:
    ...
    def "REQUEST_AUTOMATION_UPDATE for a mongodb version specified in plan"() {
        ...
        context.lastOperationJobContext = new LastOperationJobContext(plan: new Plan(parameters: [new Parameter(name: MongoDbEnterpriseProvisionState.PLAN_PARAMETER_MONGODB_VERSION, value: mongodb_version)]),
                serviceInstance: new ServiceInstance(details: [ServiceDetail.from(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_GROUP_ID, groupId),
                                                               ServiceDetail.from(ServiceDetailKey.PORT, port.toString()),
                                                               ServiceDetail.from(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_HEALTH_CHECK_PASSWORD, mongodb_enterprise_health_check_password),
                                                               ServiceDetail.from(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_HEALTH_CHECK_USER, mongodb_enterprise_health_check_user)])
        ...
        helper.getValue(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_TARGET_AUTOMATION_GOAL_VERSION) == (initialAutomationVersion + 1).toString()
        helper.getValue(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_REPLICA_SET) == deployment.replicaSet
        helper.getValue(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_MONITORING_AGENT_USER) == deployment.monitoringAgentUser
        helper.getValue(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_MONITORING_AGENT_PASSWORD) == deployment.monitoringAgentPassword
        helper.getValue(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_BACKUP_AGENT_USER) == deployment.backupAgentUser
        helper.getValue(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_BACKUP_AGENT_PASSWORD) == deployment.backupAgentPassword
    }
    ...
    def "CHECK_AUTOMATION_UPDATE_STATUS"() {
        ...
        context.lastOperationJobContext = new LastOperationJobContext(serviceInstance: new ServiceInstance(details: [ServiceDetail.from(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_GROUP_ID, groupId),
                                                                                                                     ServiceDetail.from(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_TARGET_AUTOMATION_GOAL_VERSION, automationVersion.toString())]))
        and:
    ...
    def "ENABLE_BACKUP_IF_CONFIGURED"() {
        ...
        context.lastOperationJobContext = new LastOperationJobContext(serviceInstance: new ServiceInstance(details: [ServiceDetail.from(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_GROUP_ID, groupId),
                                                                                                                     ServiceDetail.from(MongoDBServiceDetailKey.MONGODB_ENTERPRISE_REPLICA_SET, replicaSet)]))
